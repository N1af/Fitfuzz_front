...
import { SlidersHorizontal, ShoppingCart, Heart, ChevronRight, Filter, Zap } from "lucide-react";
...
const Sale = () => {
  ...
  useEffect(() => {
    const fetchPosters = async () => {
      try {
        const res = await api.get("/api/sale/posters");
        setPosters(res.data);
      } catch (err) {
        console.error("❌ Sale posters error:", err);
      }
    };
    fetchPosters();
  }, []);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("❌ Sale data fetch error:", err);
      }
    };
    fetchData();
  }, []);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const res = await api.get("/api/sale/products", { params });
            ...
            <div className="mb-8 md:mb-12">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Sale Collection</h2>
                </div>
                ...
                return (
                  ...
                  >
                    <div className="relative aspect-[3/4] overflow-hidden rounded-t-2xl">
                      {/* Discount badge - More prominent for sale */}
                      {p.discount > 0 && (
                        <div className="absolute top-3 left-3 z-10">
                          <div className="bg-gradient-to-r from-orange-500 to-red-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
                            SALE -{p.discount}%
                          </div>
                        </div>
                      )}
                      ...
                      {/* Add to cart button */}
                      <Button
                        className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-[calc(100%-2rem)] h-11 bg-gradient-to-r from-orange-600 to-red-600 text-white hover:from-red-600 hover:to-orange-600 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0 shadow-lg"
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <CardContent className="p-5">
                      ...
                      {/* Product name */}
                      <h3 className="text-sm font-semibold mb-3 text-gray-900 line-clamp-2 hover:text-orange-600 transition-colors">
                        {p.name}
                      </h3>
            ...
            {products.length === 0 && (
              <div className="text-center py-20">
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-orange-600 to-red-600 text-white px-6 py-2 rounded-full hover:shadow-lg transition-all"
                >
                  Clear all filters
                </Button>
              </div>
